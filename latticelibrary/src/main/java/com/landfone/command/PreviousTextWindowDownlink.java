...
/**
 * Method: PreviousTextWindowDownlink
 * Decription:
 * Author: raoj
 * Date: 2017/9/7
 **/
public class PreviousTextWindowDownlink extends DownlinkBaseDeviceProtocolInitiative {

    public PreviousTextWindowDownlink() {
        setCommandNumber(CommandType.Display_TextWindow);
        setCommandType(new byte[]{0x01});
        setWindowNumber(new byte[]{0x01});
        setXLayout(new byte[]{0x00,0x00});
        setYLayout(new byte[]{0x00,0x00});
        setXPoint(new byte[]{0x40,0x00});
        setYPoint(new byte[]{0x20,0x00});
        setAlternateAttribute(new byte[]{0x00,0x00});
        setStuntType(new byte[]{0x00});
        setStuntSpeed(new byte[]{0x00});
        setResidenceTime(new byte[]{0x00});
        setWhetherCleared(new byte[]{0x00});
        setContentLayout(new byte[]{0x02});
