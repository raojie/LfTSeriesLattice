 ...
 * 存折补登返回
 */
public class _04_PassbookRenewReply extends REPLY {
	...
	private String returnData = "";//补登返回数据
	...
	public int Unpack(UART_PROTOCOL ptOutPara){
		...
		if(tmpstrbb!=null){
			...
			if(tmpstrbb.length>i){
				if(tmpstrbb[i]!=null && tmpstrbb[i].equals(TradeType.PASSBOOKRENEW.getCode())){
					this.reply_type = TradeType.PASSBOOKRENEW;//指令代码
				}
			}
			i++;
			if(tmpstrbb.length>i){
			...
			if(tmpstrbb.length>i){
				this.returnData = tmpstrbb[i];//补登返回数据
			}i++;
		}
		return i;
	}
