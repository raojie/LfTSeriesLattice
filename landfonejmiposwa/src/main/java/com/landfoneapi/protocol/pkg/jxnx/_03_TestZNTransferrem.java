 ...
 */
public class _03_TestZNTransferrem extends _03_Common {
    ...
    private String payerName = "";//20

    private String keyboard = "";//1

    //构造函数
    public _03_TestZNTransferrem() {
        this.setTradeType(TradeType.TEST_ZNTRANFERREM);
    }
    ...
    public void reset() {
        ...
        try {
            ...
            this.setTrack3("");
            this.setPayerName("");
            this.setRecieverName("");
            this.setRecieverCardNo("");
        } catch (LfException e) {
    ...
    public byte[] getBytes() throws UnsupportedEncodingException {
        ...
        //拷贝数据
        i += LfUtils.memcpy(retbb, i, payerName.getBytes("GBK"), 0, payerName.getBytes("GBK").length);
        //分隔符
        i += LfUtils.memcpy(retbb, i, FS, 0, FS.length);
    ...
    public void setOperator(String operator) throws LfException {
        if (operator.getBytes().length > 15) {
            throw new LfException(-8, "operator length error, <=15");
        }
    ...
    public void setSerialNo(String serialNo) throws LfException {
        if (serialNo.getBytes().length > 20) {
            throw new LfException(-8, "serialNo length error, <=20");
        }
    ...
    public void setRecieverCardNo(String recieverCardNo) throws LfException {
        if (recieverCardNo.getBytes().length > 30) {
            throw new LfException(-8, "recieverCardNo length error, <=30");
        }
    ...
    public void setPayerName(String payerName) throws LfException {
        try {
            if (payerName.getBytes("GBK").length > 20) {
                throw new LfException(-8, "payerName length error, need <=20");
            }
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            throw new LfException(-8, "payerName getBytes(\"GBK\") error");
        }
