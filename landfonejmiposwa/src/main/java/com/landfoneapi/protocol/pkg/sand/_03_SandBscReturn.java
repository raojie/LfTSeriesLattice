...
public class _03_SandBscReturn extends _03_Common {
    ...
    public _03_SandBscReturn() {
        this.setTradeType(TradeType.SAND_BSC_RETURN);
    }
    ...
    public byte[] getBytes() throws UnsupportedEncodingException {
        ...
        //参考号
        i += LfUtils.memcpy(tmpbb, i, referNo.getBytes(), 0, referNo.getBytes().length);
        //分隔符
        i += LfUtils.memcpy(tmpbb, i, FS, 0, FS.length);
        //交易日期
        i += LfUtils.memcpy(tmpbb, i, date.getBytes(), 0, date.getBytes().length);
        //分隔符
        i += LfUtils.memcpy(tmpbb, i, FS, 0, FS.length);
        //Track2
        ...
        i += LfUtils.memcpy(tmpbb, i, FS, 0, FS.length);
        //付款码
        i += LfUtils.memcpy(tmpbb, i, paycode.getBytes(), 0, paycode.getBytes().length);
        //分隔符
        i += LfUtils.memcpy(tmpbb, i, FS, 0, FS.length);
        //预留字段0
    ...
    public void setReferNo(String referNo) throws LfException {
        if (referNo.getBytes().length > 12) {
            throw new LfException(-8, "referNo length error,need 12");
        }
    ...
    public void setDate(String date) throws LfException {
        if (date.getBytes().length > 4) {
            throw new LfException(-8, "date length error,need 4");
        }
        this.date = date;
    }
    ...
    public void setPaycode(String paycode) throws LfException {
        if (paycode.getBytes().length > 20) {
            throw new LfException(-8, "paycode length error, need <=20");
        }
