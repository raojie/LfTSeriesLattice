 ...
 */
public class _04_TestGetZNRecordInfoReply extends REPLY {
    ...
    public int Unpack(UART_PROTOCOL ptOutPara) {
        ...
        if (tmpstrbb != null) {
            ...
            if (tmpstrbb.length > i) {
                if (tmpstrbb[i] != null && tmpstrbb[i].equals(TradeType.TEST_ZNRECORDINFO.getCode())) {
                    this.reply_type = TradeType.TEST_ZNRECORDINFO;//指令代码
                }
            }
            i++;
            ...
            i++;//终端号
            if (tmpstrbb.length > i) {
                this.mer_cn_name = tmpstrbb[i];
            }
            i++;//商户中文名
            if (tmpstrbb.length > i) {
                this.mer_en_name = tmpstrbb[i];
            }
            i++;//商户英文名
            if (tmpstrbb.length > i) {
                this.bankCode = tmpstrbb[i];
            }
            i++;//收单行标识码
            if (tmpstrbb.length > i) {
                this.cardCode = tmpstrbb[i];
            }
            i++;//发卡行标识码
            if (tmpstrbb.length > i) {
                this.posCenterCode = tmpstrbb[i];
            }
            i++;//POS中心标识码
            if (tmpstrbb.length > i) {
            ...
            i++;//
            if (tmpstrbb.length > i) {
                settleDate = tmpstrbb[i];
            }
            i++;////清算日期
            if (tmpstrbb.length > i) {
                UnpredictableNum = tmpstrbb[i];
            }
            i++;////不可预知数
            if (tmpstrbb.length > i) {
                AppComuFeatures = tmpstrbb[i];
            }
            i++;//交互特征
            if (tmpstrbb.length > i) {
                TerminalPerformance = tmpstrbb[i];
            }
            i++;////终端性能
            if (tmpstrbb.length > i) {
                CardBankAppData = tmpstrbb[i];
            }
            i++;////发卡行应用数据
            if (tmpstrbb.length > i) {
                CardSerial = tmpstrbb[i];
            }
            i++;////卡片序列号
            if (tmpstrbb.length > i) {
                CardOwnerCertRst = tmpstrbb[i];
            }
            i++;////持卡人验证结果
            if (tmpstrbb.length > i) {
                RecvCardNo = tmpstrbb[i];
            }
            i++;//收款人卡号
            if (tmpstrbb.length > i) {
                RecverName = tmpstrbb[i];
            }
            i++;//收款人名字
            if (tmpstrbb.length > i) {
                PayerName = tmpstrbb[i];
            }
            i++;//付款人名字
