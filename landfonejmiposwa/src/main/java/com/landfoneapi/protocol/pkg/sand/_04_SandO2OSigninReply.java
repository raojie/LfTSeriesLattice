...
public class _04_SandO2OSigninReply extends REPLY {
    ...
    public int Unpack(UART_PROTOCOL ptOutPara) {
        ...
        if (tmpstrbb != null) {
            ...
            if (tmpstrbb.length > i) {
                if (tmpstrbb[i] != null && tmpstrbb[i].equals("SF")) {
                    this.reply_type = TradeType.SAND_O2O_SIGNIN;//指令代码
                }
            }
            i++;
            if (tmpstrbb.length > i) {
            ...
            i++;
            if (tmpstrbb.length > i) {
                code_info = tmpstrbb[i];
            }
            i++;
            if (tmpstrbb.length > i) {
                channelInfo = tmpstrbb[i];//渠道信息
            }
            i++;
            if (tmpstrbb.length > i) {
                discountInfo = tmpstrbb[i];//折扣信息
            }
            i++;
        }
        return i;
