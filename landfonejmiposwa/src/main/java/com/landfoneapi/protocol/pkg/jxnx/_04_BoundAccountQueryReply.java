 ...
 * 存折补登返回
 */
public class _04_BoundAccountQueryReply extends REPLY {
	...
	public int Unpack(UART_PROTOCOL ptOutPara){
		...
		if(tmpstrbb!=null){
			...
			if(tmpstrbb.length>i){
				if(tmpstrbb[i]!=null && tmpstrbb[i].equals(TradeType.BOUNDACCOUNT_QUERY.getCode())){
					this.reply_type = TradeType.BOUNDACCOUNT_QUERY;//指令代码
				}
			}
			i++;
			if(tmpstrbb.length>i){
				this.ReturnData = tmpstrbb[i];//商户代码
			}i++;
