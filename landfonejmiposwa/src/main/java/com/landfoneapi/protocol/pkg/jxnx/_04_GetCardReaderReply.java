 ...
 */
public class _04_GetCardReaderReply extends REPLY {
    ...
    public int Unpack(UART_PROTOCOL ptOutPara) {
        ...
        if (tmpstrbb != null) {
            ...
            if (tmpstrbb.length > i) {
                if (tmpstrbb[i] != null && tmpstrbb[i].equals(TradeType.GET_CARDREADER.getCode())) {
                    this.reply_type = TradeType.GET_CARDREADER;//指令代码
                }
            }
            i++;
            if (tmpstrbb.length > i) {
                this.outBuf = tmpstrbb[i];
            }
            i++;
        }
        return i;
