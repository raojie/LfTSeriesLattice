 ...
 * 缴费应用下载
 */
public class _03_DownloadAppcode extends _03_Common {
	...
	private String znTradeType = "";//2
	private String startDate = "";//8
	private String endDate = "";//8

	//构造函数
	public _03_DownloadAppcode(){
		this.setTradeType(TradeType.DOWNLOADPAYCODE);
	}
	...
	public byte[] getBytes() throws UnsupportedEncodingException {
		...
		//拷贝数据
		i+=LfUtils.memcpy(retbb, i, znTradeType.getBytes(), 0, znTradeType.getBytes().length);
		//分隔符
		i+=LfUtils.memcpy(retbb, i,FS,0, FS.length);

		//拷贝数据
		i+=LfUtils.memcpy(retbb, i, startDate.getBytes(), 0, startDate.getBytes().length);
		//分隔符
		i+=LfUtils.memcpy(retbb, i,FS,0, FS.length);

		//拷贝数据
		i+=LfUtils.memcpy(retbb, i, endDate.getBytes(), 0, endDate.getBytes().length);
		//分隔符
		i+=LfUtils.memcpy(retbb, i,FS,0, FS.length);
	...
	public String getZnTradeType() {
		return znTradeType;
	}

	public void setZnTradeType(String znTradeType) throws LfException {
		if(znTradeType.getBytes().length!=2){
			throw new LfException(-8,"znTradeType length error, need 2");
		}
		this.znTradeType = znTradeType;
	}
	...
	public void setStartDate(String startDate) throws LfException {
		if(startDate.getBytes().length>8){
			throw new LfException(-8,"startDate length error, need <=8");
		}
	...
	public void setEndDate(String endDate) throws LfException {
		if(endDate.getBytes().length>8){
			throw new LfException(-8,"endDate length error, need <=8");
		}
