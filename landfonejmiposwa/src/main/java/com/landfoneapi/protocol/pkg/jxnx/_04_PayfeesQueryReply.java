 ...
 * 存折补登返回
 */
public class _04_PayfeesQueryReply extends REPLY {
    ...
    public void print() {
        ...
        dbg_mPrintf("ReturnData:" + getReturnData());
    }
    ...
    public int Unpack(UART_PROTOCOL ptOutPara) {
        ...
        if (tmpstrbb != null) {
            ...
            if (tmpstrbb.length > i) {
                if (tmpstrbb[i] != null && tmpstrbb[i].equals(TradeType.PAYFEES_QUERY.getCode())) {
                    this.reply_type = TradeType.PAYFEES_QUERY;//指令代码
                }
            }
            i++;

            if (tmpstrbb.length > i) {
                this.channelType = tmpstrbb[i];//
            }
            i++;
            if (tmpstrbb.length > i) {
                this.ReturnData = tmpstrbb[i];//
            }
            i++;
