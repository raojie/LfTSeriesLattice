 ...
 */
public class _03_PreAuthorCancel extends _03_Common {
    ...
    public _03_PreAuthorCancel() {
        this.setTradeType(TradeType.PREAUTHORCANCEL);
    }
    ...
    public byte[] getBytes() throws UnsupportedEncodingException {
        ...
        retlen += authCode.getBytes().length > 0 ? (1 + authCode.getBytes().length) : 1;
        retlen += date.getBytes().length > 0 ? (1 + date.getBytes().length) : 1;
        ...
        //retlen +=1;//结束的FS
        if (commonbb != null && retlen > 0 && retlen <= (1 + 12 + 1 + 6 + 1 + 10 + 1 + 37 + 1 + 104 + 1 + 15 + 1 + 15 + 1 + 20)) {
            ...
            if (retbb != null) {
                ...
                LfUtils.memcpy(retbb, i, FS, 0, FS.length);
                i += FS.length;
                if (authCode.getBytes().length > 0) {
                    LfUtils.memcpy(retbb, i, authCode.getBytes(), 0, authCode.getBytes().length);
                    i += authCode.getBytes().length;
                }
                LfUtils.memcpy(retbb, i, FS, 0, FS.length);
                i += FS.length;
                if (date.getBytes().length > 0) {
                    LfUtils.memcpy(retbb, i, date.getBytes(), 0, date.getBytes().length);
                    i += date.getBytes().length;
                }
                LfUtils.memcpy(retbb, i, FS, 0, FS.length);
                i += FS.length;
    ...
    public void setAuthCode(String authCode) throws LfException {
        if (authCode.getBytes().length > 6) {
            throw new LfException(-8, "authCode length error ,need <=6");
        }
    ...
    public void setDate(String date) throws LfException {
        if (date.getBytes().length != 10) {
            throw new LfException(-8, "date length error,need 10");
        }
        this.date = date;
    }
