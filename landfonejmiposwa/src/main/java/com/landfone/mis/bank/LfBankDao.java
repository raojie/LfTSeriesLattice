...
import com.landfone.mis.bean.LfRequestPojo;
import com.landfone.mis.bean.LfResponsePojo;
import com.landfone.mis.bean.LfTransCfx;
...
public class LfBankDao {
    ...
    private LfResponsePojo response = new LfResponsePojo();
    private String responseStr;

    private boolean isInited = false;
    private boolean isResponse = false;

    public LfResponsePojo bankall(LfTransCfx poscfx, LfRequestPojo request) {
        Logz.i(TAG,"raoj-----bankall");
...
//                            response.setRspChin(cbmsg.info);
    ...
    public int CheckRequestIsRule(LfRequestPojo req) {
        Logz.i(TAG,"raoj---CheckRequestIsRule");
        ...
        if (transType.equals("00")) { //消费
            ...
            goodsTotalNum = req.getGoodsTotalNum();
            goodsInfo = req.getGoodsInfo();
            operId = req.getOperId();
            ...
            Logz.i(TAG,"raoj------签到4---CheckRequestIsRule");
        ...
        } else if (transType.equals("X3")) { //神思饭卡
            operId = req.getOperId();
            myApi.pos_setType(0);
            if (amount == 0) return ERR_AMT;
            ret = myApi.pos_recharge(amount);
        } else if (transType.equals("X4")) { //神思饭卡查余
            operId = req.getOperId();
            ret = myApi.pos_query();
